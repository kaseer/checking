...
} from "../../reducers/insights/InsightsActions";
import {
...
function InsightsSearch({
  ...
  const handleGTMEvent = (title) => {
    internalSearchClick(searchTerm, title, "Research");
  };
  ...
  const loadNextPage = () => {
    loadNextPageDispatch();
    triggerSearchApiCall(currentPage + 1, searchTerm, "INSIGHTS");
  };
...
const mapStateToProps = (state) => ({
  ...
  products: state.search.insightsResultsPage,
  industryFilters: state.insights.industryFilters,
  tagFilters: state.insights.tagFilters,
  topicFilters: state.insights.topicFilters,
  languageFilters: state.insights.languageFilters,
  activeFilters: state.insights.activeFilters,
  activeFilterNum: state.insights.activeFilterNum,
  hasNextPage: state.search.hasNextPageInsights,
  isAdditionalProductsLoading: state.search.isAdditionalInsightsLoading,
  totalProductCount: state.search.totalInsightsCount,
  currentPage: state.search.page,
  formatFilters: state.insights.formatFilters,
  searchTerm: state.search.query,
});

export default connect(mapStateToProps, mapDispatchToProps)(InsightsSearch);
